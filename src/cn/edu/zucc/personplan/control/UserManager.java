...
public class UserManager implements IUserManager {
	...
	public BeanUser login(String userid, String pwd) throws BaseException {
		...
		try {
			...
			int num = rs.getInt("maxerrorpwdnum");
			if (num == 0)
				throw new BaseException("您的账号已经被锁定,请找回密码，或联系管理员");
			else if (num > 0) {
				if (!pwd.equals(rs.getString("pwd"))) {
					...
					pst1.execute();
					if (num > 1)
						throw new BaseException("密码输入错误,您还有" + (num - 1) + "次机会,否则将会被锁定");
					throw new BaseException("您的账号已经被锁定,请找回密码，或联系管理员");
				}
			}
			if (rs.getInt(2) < 1)
	...
	public void changePwdtoquestion(BeanUser user, String pwdanswer, String newPwd, String newPwd2)
		...
		try {
			...
			pst.close();
			rs.close();
			sql = "update beanuser set pwd =? ,maxerrorpwdnum = 3 where userid=?";
			pst = conn.prepareStatement(sql);
			pst.setString(1, newPwd);
			pst.setString(2, user.getUserid());
			pst.execute();
